...
local function ensure_binary()
  ...
  local function find_github_url()
    local remote_file = plugin_root .. "/.git/config"
    if vim.fn.filereadable(remote_file) == 1 then
      local lines = vim.fn.readfile(remote_file)
      for _, line in ipairs(lines) do
        local url = line:match("url = https://github%.com/([^%s]+)")
        if url then
          return "https://github.com/" .. url .. "/releases/latest/download"
        end
        local ssh = line:match("url = git@github%.com:([^%s]+)")
        if ssh then
          return "https://github.com/" .. ssh .. "/releases/latest/download"
        end
      end
    end
    return nil
  ...
  local base_url = find_github_url()
